...
public class Tank {
	...
	public Tank(int x, int y, boolean good, Direction dir, TankClientMain tc) {// Tank的构造函数2
		this(x, y, good);
		this.direction = dir;
		this.tc = tc;
	}
	...
	void move() {
		...
		if (y < 40)      //防止走出规定区域
			y = 40;
		if (x + Tank.width > TankClientMain.Fram_width)  //超过区域则恢复到边界
			x = TankClientMain.Fram_width - Tank.width;
		if (y + Tank.length > TankClientMain.Fram_length)
			y = TankClientMain.Fram_length - Tank.length;
